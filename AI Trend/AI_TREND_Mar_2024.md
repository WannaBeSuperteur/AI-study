## 2024.03.31 (일)
**[왜 그렇게 판단했지? 설명 가능한 AI](https://n.news.naver.com/mnews/article/028/0002683183?sid=105)** ```AI``` ```eXplainable AI```

**설명 가능한 인공지능 (eXplainable AI, XAI)** 이란, AI의 판단에 대해 그 이유, 즉 **AI가 그렇게 판단하기까지의 과정** 을 '설명'할 수 있어야 한다는 개념 및 이와 관련된 기술이다.
* XAI는 1970년대의 AI 기술인 **전문가 시스템** (전문 지식들을 논리적으로 연산하여 결론을 도출하는 AI) 에서부터 등장했는데, 이와 관련하여 전문가 시스템이 사용자에게 추가 질문을 하는 경우 **왜 그 질문을 했는지 설명하면 사용자가 시스템을 더 신뢰** 하는 현상이 발견되었다.
* 21세기에 딥 러닝에 대한 연구가 다시 활발해지면서 **딥러닝의 블랙박스 성질** 때문에 XAI에 대한 관심이 다시 높아졌다.
  * 특히 **자율주행 자동차** 등 AI의 신뢰성이 크리티컬하게 작용하는 분야에서 AI의 신뢰성에 대한 문제에 대한 공감대가 높았다.
* 최근에는 XAI와 관련하여 다음과 같은 분야의 기술이 개발되고 있다.
  * 학습된 모델에서 특정 학습 데이터의 영향력 판단
  * 모델이 출력한 output에 대해, 그 output을 도출하는 데 큰 영향을 미친 변수 파악
* XAI 분야에서 해결해야 할 문제는 **설명이란 무엇이고, 그것을 통해 달성해야 하는 것은 무엇인가** 이다.  
