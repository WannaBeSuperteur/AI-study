## 2025.06.30 (월)
**[네이버, 추론형 모델 '하이퍼클로바 X 씽크' 공개](https://n.news.naver.com/mnews/article/277/0005615062?sid=105)** ```AI``` ```Naver AI``` ```Large Language Model```

* 2025.06.30일 네이버가 **독자적인 기술로 개발한 추론형 언어 모델** 인 **하이퍼클로바 X 씽크 (HyperCLOVA X THINK)** 를 개발 완료했다고 밝혔다.
  * 이와 함께 해당 모델의 기술 보고서 (테크니컬 리포트) 도 발표했다.
* 하이퍼클로바X 씽크의 성능 수준은 다음과 같다.

| 벤치마크                 | 성능 수준                                        |
|----------------------|----------------------------------------------|
| KoBALT-700 (코발트 700) | 비슷한 규모의 국내 & 글로벌 최고 수준 LLM 에 비해 **높은 점수**    |
| HAERAE-Bench         | KoBALT-700 과 마찬가지로, 주요 오픈소스 LLM 보다 **높은 점수** |

* 하이퍼클로바X 씽크의 특징은 다음과 같다.
  * 높은 수준의 언어 이해 및 추론 능력
  * **시각 정보 바탕** 추론 능력 (수능 생명과학의 생태계 천이 과정 그림 및 각종 그래프 등)
* 한편, 네이버는 추론 모델에 대한 **오픈소스 공개 계획** 도 있다.
