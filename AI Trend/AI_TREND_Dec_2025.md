...
* 정부가 AI를 바이오 분야 및 신소재 발굴 등에 적용한다.
  * 구체적으로는 **식/의약품 안전관리에도 AI를 적용** 하여, **AI 기반으로 위험 요인 등을 사전 예측 및 제거** 한다. 
* 이와 관련하여 심의/의결된 안건은 다음과 같다.
  * ```제4차 국가생명연구자원 관리활용 기본계획 (2026~2030)```
    * AI바이오의 핵심인 '생명연구자원' 관련 **3대 전략** 및 **10대 중점 추진 과제** 제시
  * ```제1차 식품·의약품 규제과학혁신 기본계획 (2026~2030)```
* 특히 생명연구자원 관련 3대 전략은 다음과 같다.
  * 숨은 바이오 소재 확보 & 산업 활용 활성화
  * AI 기반 바이오 데이터 확보/관리/활용 등
  * 민-관 협력 공고화를 통한 AI 바이오 혁신 지원

## 2025.12.29 (월)
**[네이버, 텍스트/이미지/음성 동시 이해 옴니모달 AI 모델 공개](https://n.news.naver.com/mnews/article/003/0013680725?sid=105)** ```AI``` ```Multimodal AI``` ```Naver AI```

* 2025.12.29일 네이버가 **텍스트, 이미지, 음성을 동시에 이해할 수 있는 AI 모델** 인 **하이퍼클로바 X 시드 8B 옴니** 를 공개했다.
  * 텍스트, 이미지, 음성을 동시에 이해할 수 있으므로, 멀티모달을 넘어선 **옴니모달 모델** 로 불린다.
  * 이 모델은 **파운데이션 AI 모델** 이며, **오픈소스로 공개** 되었다.
* 이 모델의 특징은 다음과 같다.
  * 멀티모달 (단일 모달리티 중심 학습 후 다른 모달리티 처리 결합) 형태를 넘어선 **옴니모달 (처음부터 모든 모달리티의 데이터 학습)** 모델
  * 텍스트 지시 기반 이미지 생성 및 편집 가능
* 이 모델의 성능 수준은 다음과 같다.
  * 2026 수능 국어, 수학, 영어, 한국사 모두 1등급 수준 (영어, 한국사 만점)
  * 도구 활용 (AI 에이전트로서의 핵심 역량) 등 **복합적 문제 해결 역량** 보유

## 2025.12.28 (일)
**[SKT, 국내 최초 500B 파라미터 LLM 공개](https://n.news.naver.com/mnews/article/382/0001244633?sid=105)** ```AI``` ```Large Language Model```

* 2025.12.30일 과학기술정보통신부의 '독자 AI 파운데이션 모델 프로젝트 1차 발표회'에서 SKT가 국내 최초로 **500B (5000억 개) 파라미터를 가진 LLM을 공개** 할 예정이다.
  * 해당 LLM의 이름은 **A.X K1** 이다.
* A.X K1의 특징은 다음과 같다.
  * 추론 (inference) 시 **약 330억 개 (33B) 파라미터가 활성화됨** (최대한 경량화)
  * **한국어 특화**
  * 에이닷 기반으로 해당 모델을 제공하여, **전 국민이 쉽게 AI를 이용할 수 있는 환경** 구축
* 한편, A.X K1은 단순 기술 성과가 아닌 **SK 그룹사 및 컨소시엄 중심으로 확산** 되어 영향력을 발휘할 예정이다.
* 또한, A.X K1은 **오픈소스 공개 및 API가 제공** 되어 **AI 에이전트 개발 환경** 제공에 기여할 예정이다.

## 2025.12.27 (토)
**[SOOP (아프리카TV), 스트리머 대신 방송하는 AI '쌀사 2.0' 공개](https://n.news.naver.com/mnews/article/003/0013679378?sid=105)** ```AI```

* SOOP (아프리카TV) 이 **스트리머가 자리 비움 상태일 때 대신 방송을 진행하는 AI** 인 **쌀사 (SARSA) 2.0** 을 공개했다.
  * 즉, **스트리머와 이용자를 연결하는 매니저** 역할을 수행하는 셈이다.
* SOOP의 기능은 다음과 같다.
  * 스트리머의 자리비움 시 **스트리머 대신 방송 흐름이 끊기지 않게 진행 (채팅 반응, 리액션 등)**
  * 방송 화면의 물체 (상품 등) 인식을 통한 콘텐츠-광고 연결 환경 제공이 향후 목표
* 한편, SOOP에는 쌀사 외에도 **싸빅 (SAVYG, 고품질 영상 제작 AI), 수피 (SOOPi, 유저 맞춤 비서)** 등의 AI 서비스가 있다.

## 2025.12.26 (금)
**[전문가 혼합 (MoE), LLM 안전성 훼손한다](https://n.news.naver.com/mnews/article/079/0004099384?sid=102)** ```AI``` ```Large Language Model```

* KAIST 공동연구팀이 **거대 언어 모델 (LLM) 의 전문가 혼합 (MoE) 구조를 악용한 공격 기법** 을 규명했다.
  * 이것은 **세계 최초의 규명** 에 해당하며, 이 성과로 **ACSAC 2025 최우수논문상** 을 수상했다.
* 핵심 내용은 다음과 같다.
  * 여러 개의 전문가 모델 중 **1개만 오픈소스로 공개될 경우, 이를 통해 전체 LLM이 위험한 응답을 생성** 하도록 할 수 있다.
  * 특정 상황에서 해당 LLM이 반복적으로 선택되는 경우가 위험하다. 
    * 해당 상황에서 **전체 LLM의 성능이 거의 저하되지 않아서 문제를 인지하기 어렵다** 는 것도 위험 요소이다.
* 실험 결과, **해당 공격 기법을 통해 위험한 응답 발생률이 최대 80%** 에 이르게 할 수 있다고 한다.

## 2025.12.25 (목)
**[AI에게 크리스마스 선물 추천 요청해 봤더니](https://n.news.naver.com/mnews/article/037/0000037459?sid=101)** ```AI``` ```Large Language Model```

* ChatGPT와 Gemini를 대상으로 **크리스마스 선물 추천 요청** 질문에 대한 응답 결과는 다음과 같다.
  * 비교 대상 LLM은 **쇼핑 어시스턴트 (ChatGPT-5.2), 구글 제미나이 프로** 이다. 
* 양쪽의 추천 결과를 요약하면 다음과 같다.

|               | 챗GPT (쇼핑 어시스턴트)            | 제미나이 프로               |
|---------------|----------------------------|-----------------------|
| 사용자 상호작용 방식   | **추가 질문** 생성 (감성 vs 실용성 등) | 추가 질문 없이 **즉시 답변 생성** |
| 응답 시간         | 약 9분 **(느림)**              | 즉시 응답 **(빠름)**        |
| 응답 결과의 길이     | 약 4000자 (글, 구매 링크 포함)      | 표 형태로 정리              |
| 품목 지정 후 질문 결과 | 마찬가지로 추가 질문 생성             | 마찬가지로 즉시 추천 리스트 제시    |
