...
* Main Model 만 사용하고, Embedding Layer는 S-BERT 및 자체 임베딩 (임베딩 모델) 을 이용하는 대신 Main Model에 TensorFlow Keras 의 Embedding Layer ([공식 문서](https://www.tensorflow.org/api_docs/python/tf/keras/layers/Embedding)) 를 추가한다.
  * output과 각 embedding에 대해 output과 동일한 크기의 weight array를 각각 dot product 해서 거리를 구하는, 위의 **최종 채택** 된 방식과 함께 적용할 수 있다.
* 자주 등장하는 단어 token일수록 위 방법으로 구한 각 embedding 과의 거리에 더 작은 값을 추가 가중치로 곱해서 (예: ```1 / (해당 token의 전체 dataset에서의 등장 횟수)```) 그 값을 거리로 이용한다.
  * 즉, **output vector 와 비슷한 거리만큼 떨어진 embedding 에 해당** 하는 여러 단어 중 **더 자주 등장하는 단어가 더 잘 선택** 되게 한다.
* 거리를 구하는 방법을 Euclidean Distance 대신 코사인 유사도를 이용한다. 다른 가중치는 동일하게 적용한다.
