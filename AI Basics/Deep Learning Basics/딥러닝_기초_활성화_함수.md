...
* 각 레이어 종류 (Conv. 및 Fully Connected) 별로 적용할 활성화 함수의 종류 및 관련 상수, learning rate를 하이퍼파라미터로 하여, MNIST 데이터셋을 대상으로 하이퍼파라미터 최적화를 실험한다.
...
Loss Function 과 마찬가지로 활성화 함수 역시 잘못 사용하면 모델 학습이 잘 이루어지지 않는 오류가 발생한다. 자세한 것은 [해당 문서](딥러닝_기초_활성화_함수_Misuse.md) 참고.