...
* 수식
  * ${E[g^2]}t = p * {E[g^2]}_{t-1} + (1 - p) * g_t^2$
...
**2. 각 Optimizer 별 테스트 데이터셋 최종 성능 (정확도) 및 최적 하이퍼파라미터**

| Optimizer | 최종 정확도     | HPO Valid set 최고 정확도 | 최적 하이퍼파라미터                                                                                                  |
|-----------|------------|----------------------|-------------------------------------------------------------------------------------------------------------|
...
**3. 각 Optimizer 별 하이퍼파라미터 최적화 진행에 따른 정확도 추이**
...
**4. 각 Optimizer 별 특정 하이퍼파라미터의 값에 따른 성능 분포**
...
* **overfitting 이 아닌, 학습이 아예 되지 않는 현상의 발생 여부는 불분명하다.**