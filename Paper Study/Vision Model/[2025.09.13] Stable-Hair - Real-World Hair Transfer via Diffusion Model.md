...
| 실험             | 결과 요약                                                                                                                     |
|----------------|---------------------------------------------------------------------------------------------------------------------------|
| 양적 실험          | CLIP-I, FID, SSIM, IDS 성능지표가 **다른 모든 모델에 비해 우수**<br>- 단, **PSNR** 의 경우 **Hairclip v2** 모델이 가장 우수                          |
| 질적 실험          | Stable-Hair 로 생성한 이미지가 가장 자연스럽고 정확도가 높음 (User Study 결과도 동일하게 나타남)                                                         |
| Ablation Study | - **Latent ControlNet** 을 사용했을 때의 성능이 원래의 ControlNet 사용 시보다 좋음<br>- **이미지뿐만 아니라 동영상 데이터 포함** 시, 포즈와 헤어스타일 변화가 조금 더 자연스러워짐 |
...
| 구현 사항                           | 설정값                                  |
|---------------------------------|--------------------------------------|
| **Pre-trained** Diffusion Model | Stable Diffusion V1-5                |
| Inference                       | Training 과 동일한 two-stage approach 적용 |
| Sampler                         | DDIM sampler                         |
| sampling steps                  | 30                                   |
| classifier-free guidance scale  | 1.5                                  |

* Stage 1 (Bald Converter 학습)

| 구현 사항                                                                               | 설정값                  |
|-------------------------------------------------------------------------------------|----------------------|
| GPU                                                                                 | **Single** H800 GPU  |
| batch size                                                                          | **16**               |
| [Learning Rate](../../AI%20Basics/Deep%20Learning%20Basics/딥러닝_기초_Learning_Rate.md) | **5e-5** (= 0.00005) |
| total steps                                                                         | 8,000                |

* Stage 2 (Hair Extractor & Latent IdentityNet 학습)
  * **Stage 1 에서 준비한 Triplet Data** 를 이용하여 학습 실시
...
| 기존 모델 / 방법론 | 논문 (발표 연도)                                                                                                                                                 |
|-------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Barbershop  | [Barbershop: GAN-based Image Compositing using Segmentation Masks (2021)](https://arxiv.org/pdf/2106.01505)                                                |
| SYH         | [Style Your Hair: Latent Optimization for Pose-Invariant Hairstyle Transfer via Local-Style-Aware Hair Alignment (2022)](https://arxiv.org/pdf/2208.07765) |
| HairFastGAN | [HairFastGAN: Realistic and Robust Hair Transfer with a Fast Encoder-Based Approach (2024)](https://arxiv.org/pdf/2404.01094)                              |
| Hairclip    | [HairCLIP: Design Your Hair by Text and Reference Image (2022)](https://arxiv.org/pdf/2112.05142)                                                          |
| Hairclip v2 | [HairCLIPv2: Unifying Hair Editing via Proxy Feature Blending (2023)](https://arxiv.org/pdf/2310.10651)                                                    |

* 비교 대상 Metric

| Metric      | 사용 목적                                               |
|-------------|-----------------------------------------------------|
| FID         | 서로 다른 관점의 헤어스타일 변환 성능 측정                            |
| SSIM        | 원본 이미지와 생성된 이미지 간의 **identity & 배경 정보 일치도** 측정      |
| PSNR        | SSIM 과 동일                                           |
| Insightface | 원본 이미지와 생성된 이미지 간의 **identity similarity (IDS)** 측정 |
| CLIP-I      | 헤어스타일 변환 성능 측정                                      |

### 4-3. 실험 결과 (기본)

**1. 양적 비교**

* CLIP-I, FID, SSIM, IDS 모두 **Stable-Hair** 가 가장 우수
* **PSNR** 의 경우 **Hairclip-v2** 가 가장 우수

![image](../images/StableHair_7.PNG)

[(출처)](https://ojs.aaai.org/index.php/AAAI/article/view/33123/35278) : Yuxuan Zhang and Qing Zhang et al., "Stable-Hair: Real-World Hair Transfer via Diffusion Model"

**2. 질적 비교**

* **Hairclip-v2** 로 생성한 헤어스타일 변환 이미지의 경우, 다른 기법에 비해 **상당히 자연스러움**

![image](../images/StableHair_8.PNG)

[(출처)](https://ojs.aaai.org/index.php/AAAI/article/view/33123/35278) : Yuxuan Zhang and Qing Zhang et al., "Stable-Hair: Real-World Hair Transfer via Diffusion Model"

* **User Study** 결과
  * **Accuracy, Preservation, Naturalness** 의 3가지 지표 모두 **Stable-Hair 가 가장 우수**

| 구분           | 설명                              |
|--------------|---------------------------------|
| Accuracy     | 헤어스타일 변환의 정확도                   |
| Preservation | **hair 외의 영역을 보존** 하는 정도        |
| Naturalness  | 생성된 이미지의 **시각적인 사실성 (= 자연스러움)** |

![image](../images/StableHair_9.PNG)
...
| 구분            | 설명                             |
|---------------|--------------------------------|
| Ours (latent) | **Latent ControlNet** 을 이용한 결과 |
| Ours (pixel)  | **원래의 ControlNet** 을 이용한 결과    |

* 모든 성능지표에서 **Latent ControlNet** 을 사용했을 때의 결과가 원래의 ControlNet 사용 시에 비해 우수하다.

![image](../images/StableHair_10.PNG)

[(출처)](https://ojs.aaai.org/index.php/AAAI/article/view/33123/35278) : Yuxuan Zhang and Qing Zhang et al., "Stable-Hair: Real-World Hair Transfer via Diffusion Model"

**2. Visual Ablation (Image vs. Image + Video)**

* 학습 프로세스에 이미지뿐만 아니라 **동영상 데이터 포함 시** 다음과 같은 장점이 있다.
  * 포즈 (pose) 가 좀 더 robust 해짐
  * 헤어스타일 변화가 조금 더 자연스러워짐

![image](../images/StableHair_11.PNG)
