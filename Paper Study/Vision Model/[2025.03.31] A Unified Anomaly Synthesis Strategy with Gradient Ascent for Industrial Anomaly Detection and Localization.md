...
* 이 Distance 를 나타내는 $\widetilde{g}_i^{h,w}$ 를 $r_1 < \vert \vert \widetilde{g}_i^{h,w} - u_i^{h,w} \vert \vert_2 < r_2$ 의 거리가 되게 하기 위해, 다음과 같이 **Truncated Distance 인 $\hat{\eta}_i^{h,w}$ 를 이용한다.**
