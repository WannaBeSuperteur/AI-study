...
* 다음을 $t$ 회 반복한다.
  * $A_t = (\theta_t, \Sigma_t)$ 에 대해 **generative distribution $P_gen(x, y | A_t)$** 이 유도된다.
  * 이 generative distribution 으로부터 **intrinsic dataset $D_gen^t$** 을 샘플링한다.
  * **quality-control operator (QC)** 가 이 데이터셋의 example 등을 **필터링 또는 가중치 부여** 한다.
  * 최종 데이터셋은 $D_t = D_t^{base} \cup QC(D_t^{gen})$ 으로 한다.
  * 파라미터를 업데이트한다. ($\theta_{t+1} = IMPROVE_\theta(\theta_{1:t}; D_t)$)
