...
* [4. Scaffolding Improvement](#4-scaffolding-improvement)
  ...
  * [4-4. Full Scaffolding](#4-4-full-scaffolding)
* [5. 응용 분야](#5-응용-분야)
...
**현대 AI 에이전트의 Self-improving** 에 대한 2가지 접근 방법은 다음과 같다.
...
![image](../images/AgentSurvey_5.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

### 4-1. Prompt (프롬프트)

LLM의 프롬프트는 **Agent의 행동을 결정하는 핵심 요소** 이므로, 프롬프트 최적화는 **scaffolding improvement 중 가장 핵심적이고, 동시에 접근이 쉬운** 부분이다.

* 프롬프트 개선 방법은 다음과 같이 구분할 수 있다. 이때 **$S_t$ 는 learning signal 을 의미** 한다.

| 프롬프트 개선 방법                      | 설명                                                                |
|---------------------------------|-------------------------------------------------------------------|
| Scalar-Feedback Optimization    | $S_t$ 는 **scalar 형태의 performance score (reward, accuracy 등)** 이다. |
| Qualitative-Feedback Refinement | $S_t$ 는 **자연어 형태로 구성된 제안 또는 비판** 이며, 이는 **해석 가능한 수정 가이드** 이다.     |
| Population-Based Evolution      | $S_t$ 가 **후보 프롬프트 집합** 에 대한 **평가 및 선택 signal** 로 구성된 경우           |
| Textual Gradient Optimization   | $S_t$ 가 **방향성에 대한 가이드 (textual gradient 등)** 인 경우                 |

![image](../images/AgentSurvey_6.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

![image](../images/AgentSurvey_7.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

### 4-2. Memory (메모리)

LLM Agent 에게 메모리 (Memory) 는 **장기간의 에이전트 행동을 위한 핵심적인 인지적 scaffold** 이다.

* Self-improving agent는 메모리를 **단순한 저장소가 아닌, 능동적으로 진화 가능한 대상** 으로 간주한다.
* Agent 들은 **새로운 경험에 의해 memory representation을 자동 업데이트 (재구성 & 확장)** 한다.

Memory-based imporvement 는 다음과 같은 3가지 축으로 구성된다.

| 축 구분              | 설명                               |
|-------------------|----------------------------------|
| Memory Objects    | 저장된 정보의 단위                       |
| Memory Structure  | 메모리 정보에 대한 인덱싱 스키마 및 토폴로지 구조     |
| Memory Processing | 메모리 업데이트 메커니즘 (생성, 탐색, 수정, 삭제 등) |

Memory evoluation은 다음과 같은 수식으로 나타낼 수 있다.

* $m_{t+1} = IMPROVE_m(m_t; S_t)$

![image](../images/AgentSurvey_8.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

**Memory Object** 는 **저장된 정보의 단위** 로, 다음과 같이 구분한다.

| Memory Object 구분 | 설명                                                                                                         | 핵심 장점                                                 |
|------------------|------------------------------------------------------------------------------------------------------------|-------------------------------------------------------|
| Explicit Objects | - **인간이 읽을 수 있고 직접 조작 가능** 한 형태<br>- 따라서 해석 가능성, 안전성 등이 중요할 때의 default choice 가 됨                          | 컨트롤 가능성 (개발자가 수정 가능)                                  |
| Implicit Objects | - 메모리를 **machine-native 한 latent representation (latent vector)** 형태로 저장<br>- latent token, hidden state 등 | compactness (context limit, latent budget 이 있는 경우 유리) |

### 4-3. Tool (도구)

기본적인 LLM Agent는 **사람에 의해 정해진 static 한 toolkit list** 에 의존해야 했고, **새로운 과제에 대해 새로운 tool을 탐색하거나, 기존의 toolkit list 를 업데이트** 하지는 못함

* Self-improving Agent 는 이러한 점을 극복하여, **Tool Governance Metacognition** 으로 진화

**Tool Governance Metacognition** 의 메커니즘은 다음과 같다.

| 메커니즘                      | 설명                                                                                                                                                                                                                                                       |
|---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Dynamic Tool Routing      | Agent가 **tool 들의 선택, 실행 순서 결정 등을 어떻게 하는지** 에 대한 것으로, 다음과 같이 분류<br>- **Retrieval- and graph-based** routing<br>- **Policy-learning** routing (도구 선택을 순차적 의사 결정으로 내재화)<br>- **Proactive and interactive** routing (routing 을 1회성 의사결정이 아닌, 상호 작용 프로세스로 처리) |
| Iterative Tool Refinement | **오류 발생 가능한 프로그램** 을 **신뢰 가능한 skill** 로 변환<br>- 도구 오류는 **에이전트 실행 시에만 문제가 되는 것이 아님**                                                                                                                                                                      |
| Autonomous Tool Creation  | 기존에 실행 가능한 tool이 충분하지 않을 때, **Agent 스스로 실행 가능한 함수를 제작** 하는 것                                                                                                                                                                                             |

### 4-4. Full Scaffolding

**Full Scaffolding** 은 가장 진화된 형태의 Self-improvement 로, **전체적인 운영 로직, 구조 및 코드베이스를 수정 가능한 상태** 로 간주하는 것이다.

* 즉, Agent가 **근본적인 구조 자체를 스스로 업데이트** 하는 것을 말한다.

이를 수식으로 나타내면 다음과 같다.

* $A_t = (\theta_t, \Sigma_t)$
* $Sigma_{t+1} = IMPROVE_\Sigma (\Sigma_t; S_t)$
* $Sigma_{t+1} = I_{\Sigma_t} (\Sigma_t; S_t)$
* 여기서 $I$ 는 **Agent를 진화시키는 improver 역시 함께 improve 될 수 있음** 을 의미한다.

![image](../images/AgentSurvey_9.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

## 5. 응용 분야

Self-improving Agent 의 응용 분야는 다양하다.

| 응용 분야          | Self-improving Agent의 장점                        | 관련 branch                                      |
|----------------|-------------------------------------------------|------------------------------------------------|
| 소프트웨어 개발/엔지니어링 | 컴파일러, 단위 테스트, linter 등을 Agent화 가능               | FM (Foundation Model), scaffolding improvement |
| 웹 탐색 및 자동화     |                                                 | FM, scaffolding (both)                         |       
| 게임/전략적 추론      | 반복적인 상호작용, 잘 정의된 목표 등으로 인해 적응 빠름                | FM (강화학습), scaffolding (계획, 메모리 등)             |
| 과학적 발견         | 전체 연구 라이프사이클 (이론 도출, 실험 설계, 분석, 커뮤니케이션 등) 적용 가능 | scaffolding                                    |
| 로보틱스 등         | 연속된 상태 (행동 공간), 시뮬레이션과 현실의 차이 등으로 인해 적용 가능      | FM, scaffolding (both)                         |
| 일반적인 컴퓨터 조작    | 재사용 가능한 과정을 적용하여, 새로운 앱 상호작용 학습                 | FM, scaffolding (both)                         |

![image](../images/AgentSurvey_10.PNG)

[(출처)](https://arxiv.org/pdf/2607.13104) : Zhe Ren and Yimeng Chen et al., "Self-Improvements in Modern Agentic Systems: A Survey"

## 6. 평가 방법 및 벤치마크

Self-improving Agent 를 평가하는 방법은 다음과 같다.

| 평가 방법 구분                 | 설명                                                                                                                                                                                                                                                                     |
|--------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Metric-Based Measurement | **결정적이고 (deterministic), 실행 가능한** evaluator 사용                                                                                                                                                                                                                         |
| Judge-Based Measurement  | **parameterized evaluator** 사용 (사전에 정해진 평가 기준 및 보조 모델 $\theta_{judge}$ 사용)                                                                                                                                                                                             |
| Mechanism Benchmarks     | 엄격한 평가 프로토콜을 위한 테스트베드로서 기능하는 벤치마크<br>- **FM-Level Evaluation** : 모델 파라미터 변화 ( $\theta_t → \theta_{t+1}$ ) 에 의한 Agent의 역량 향상 측정 (scaffold 는 그대로 둠)<br>- **Scaffold-Level Evaluation** : Scaffold 변화 ( $\Sigma_t → \Sigma_{t+1}$ ) 에 따른 Agent의 역량 향상 측정 (모델 파라미터는 그대로 둠) | 
| Domain Benchmarks        | 소프트웨어 개발/엔지니어링, 웹 탐색, 게임/전략적 추론 등에서 다양한 벤치마크 존재                                                                                                                                                                                                                        |

## 7. 논의 사항

Self-improving Agent와 관련하여 다음과 같이 논의할 법한 사항들이 있다.

* 시스템 설계에서의 시사점
  * Operational Scaffold를 수정하는 것은 **Agent를 빠르게 업데이트** 하는 것에 해당하지만, 파라미터 $\theta$ 에 대한 업데이트는 **훨씬 느리다.**
    * 프롬프트를 잘못 짠 것은 쉽게 되돌릴 수 있지만, **모델 자체의 파라미터를 잘못 학습한 것은 쉽게 추적하기 어렵다.**
  * Agent는 critic의 비판에 대해 **가장 짧은 보상 경로** 를 학습하여, 결국 Agent의 역량 향상에 한계가 있을 수 있다.
  * Agent가 자신의 운영 로직 또는 소스코드를 수정함에 따른 리스크

* 미래 방향성
  * 테스트 시점의 추가적인 적응 메커니즘 (학습 시점과 배포 시점의 동작 차이를 줄여서 **실제 환경에 잘 적응** 할 수 있도록 해야 함) 
  * 인간이 지정한 task를 수동적으로 수행하기보다는, **가치 있는 task를 Agent 스스로 발굴** 하는 방향성
  * **자원이 제한된 환경** 에서의 improvement
  * **여러 개의 Agent가 협력하면서 함께 진화** 하는 형태의 improvement
  * Data Drift 로부터 살아남는 Agent
