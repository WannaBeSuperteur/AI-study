...
* SelfExtend는 성능 향상의 장점도 있지만, **모든 Query-Key pair 에 대한 Extra Attention** 때문에 구현 시 **연산량이 증가** 한다.
  * blocked kernel 과 같은 방법으로 최적화할 수 있다.
* Group Size 가 크면 모델 성능이 떨어질 수 있다.
* SelfExtend 는 정보 보존을 위해 여전히 **전체 sequence 를 처리** 한다.
  * 단, 이 부분은 **prompt compression** 등을 통해 연산량을 줄일 수 있다.
* long context 처리 능력을 어떻게 평가할지는 추가 논의가 필요하다.
* 향후 ```FLOOR``` 보다 정교한 mapping 메커니즘을 생각해 볼 수 있다.
