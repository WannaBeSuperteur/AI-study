...
| Multi-head Attention                   | Query, Key, Value 를 **여러 개의 head 로 분리하고, 각 head 에서 Attention 함수를 병렬 연산**<br>- 이 결과들은 나중에 concatenate 됨                     | $MultiHead(Q, K, V) = Concat(head_1, ..., head_h)W^O$<br>($head_i = Attention(QW_i^Q, KW_i^K, VW_i^V)$)                                      |
