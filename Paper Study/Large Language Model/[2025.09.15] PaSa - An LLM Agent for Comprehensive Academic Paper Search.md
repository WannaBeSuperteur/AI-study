...
* [5. 실험 및 그 결과](#5-실험-및-그-결과)
  * [5-1. 실험 설정](#5-1-실험-설정)
  * [5-2. 평가 벤치마크 데이터셋 및 비교 대상 모델](#5-2-평가-벤치마크-데이터셋-및-비교-대상-모델)
...
* 즉, **Selector 를 Crawler 의 보조적인 reward model** 로 사용하는 셈이다.

### 4-3. Selector

Selector 는 **다음 2가지를 입력받아서 또 다른 2가지를 출력하는 LLM Agent** 이다.

| 구분 | 입력/출력                                | 설명                                                                                                       |
|----|--------------------------------------|----------------------------------------------------------------------------------------------------------|
| 입력 | scholar query (학술적 쿼리)               |                                                                                                          |
| 입력 | research paper (연구 논문)               |                                                                                                          |
| 출력 | 단일 결정 토큰 $d$                         | ```True``` 또는 ```False```                                                                                |
| 출력 | rationale $r = (r_1, r_2, ..., r_m)$ | $d$ 의 결정을 뒷받침하는 $m$ 개의 토큰<br>- **결정의 정확도** 를 높이고,<br>- 사용자에게 그 **결정에 대한 이유를 설명** 함으로써 **신뢰도를 높이기** 위해 사용 |

* 이때 Crawler 학습을 효율적으로 진행하기 위해, **Decision Token 은 rationale 이전에 나타난다.**
  * 이를 통해 Selector 는 Crawler 학습 중 **단일 토큰 보상 모델** 로서 기능한다.
...
* Crawler 와 Selector 모두 **Qwen2.5-7B** 기반
* 이것을 **PaSa-7B** 라고 칭함
* [Supervised Fine-Tuning (SFT)](../../AI%20Basics/LLM%20Basics/LLM_기초_Fine_Tuning_SFT.md) 학습 설정값

| 구분                                                                                  | 설정값                 |
|-------------------------------------------------------------------------------------|---------------------|
| epochs                                                                              | 1 epoch             |
| [Learning Rate](../../AI%20Basics/Deep%20Learning%20Basics/딥러닝_기초_Learning_Rate.md) | 1e-5 (= 0.00001)    |
| batch size                                                                          | 4 (per device)      |
| GPU                                                                                 | 8 x NVIDIA H100 GPU |
...
* Stage 2. PPO training
  * 학습 안정성을 위해, Policy Model 을 Freeze 시킨 후 **Value Model 만 학습 실시**
  * 이후 **Policy Model 과 Value Model 을 함께 학습**

### 5-2. 평가 벤치마크 데이터셋 및 비교 대상 모델
...
| 비교 대상 모델/방법론       | 설명                                        | LLM 사용   | 웹 검색 |
|--------------------|-------------------------------------------|----------|------|
| Google             | **Google** 을 이용하여 query 를 직접 검색           | X        | O    |
| Google Scholar     | **Google Scholar** 를 이용하여 query 를 직접 검색   | X        | O    |
| Google with GPT-4o | **GPT-4o 로 쿼리 의역** → 의역된 쿼리로 검색           | GPT-4o   | O    |
| ChatGPT            | query 를 **ChatGPT (GPT-4o 웹 검색 기능)** 에 질문 | GPT-4o   | O    |
| GPT-o1             | query 를 **GPT-o1 (추론형 모델)** 에 질문 (웹 검색 X) | o1 (추론형) | X    |
| PaSa-GPT-4o        | PaSa 를 구현하되, **GPT-4o 를 이용한 프롬프팅** 실시     | GPT-4o   |      |
...
| 실험                        | 결과 요약                                                                                 |
|---------------------------|---------------------------------------------------------------------------------------|
| AutoScholarQuery test set | ```PaSa-7b``` & ```PaSa-7b-ensemble``` 이 **Precision 을 제외한 나머지 성능지표에서 다른 모든 방법보다 우수** |
| RealScholarQuery test set | ```PaSa-7b``` & ```PaSa-7b-ensemble``` 이 **모든 성능지표에서 다른 모든 방법보다 우수**                  |
| Selector Evaluation       | **F1 Score** 기준으로, ```PaSa-7b``` & ```Pasa-7b-ensemble``` 이 나머지 방법보다 우수               |

* AutoScholarQuery & RealScholarQuery test set 결과

![image](../images/PaSa_13.PNG)

[(출처)](https://arxiv.org/pdf/2501.10120) : Yichen He and Guanhua Huang et al., "PaSa: An LLM Agent for Comprehensive Academic Paper Search"

* Selector Evaluation 결과

![image](../images/PaSa_14.PNG)
...
| 실험                                                               | 데이터셋                                        | 결과 요약                                                                                                                                                                           |
|------------------------------------------------------------------|---------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ```[Expand]```, 강화학습, 'Selector as Reward Model (RM)' 각각 제외 시 비교 | **AutoScholarQuery** & **RealScholarQuery** | **Recall** 및 **Crawler Recall** 비교<br>- ```[Expand]``` 제외 시 < 강화학습 제외 시 < Selector as RM 제외 시 < **모두 포함 시**<br><br>**Precision** 비교<br>- PaSa-7b 에서 **무엇인가를 제외한 경우** 가 성능이 더 좋음 |
| $\alpha$ (reward coefficient) 값에 따른 성능 비교                        | **AutoScholarQuery** only                   | $\alpha$ 가 증가할수록,<br>- **Crawler Recall, Crawler Actions, Recall** 증가<br>- **Precision** 감소                                                                                     |

* ```[Expand]```, 강화학습, 'Selector as Reward Model (RM)' 각각 제외 시 상세 비교

![image](../images/PaSa_15.PNG)

[(출처)](https://arxiv.org/pdf/2501.10120) : Yichen He and Guanhua Huang et al., "PaSa: An LLM Agent for Comprehensive Academic Paper Search"

* $\alpha$ (reward coefficient) 값에 따른 성능 상세 비교

![image](../images/PaSa_16.PNG)
