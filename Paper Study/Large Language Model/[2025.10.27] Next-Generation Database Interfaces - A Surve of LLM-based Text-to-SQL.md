...
| 논의 사항                                            | 요약                                                                                 |
|--------------------------------------------------|------------------------------------------------------------------------------------|
| [LLM 기반 방법 vs. 전통적 방법](#4-1-llm-기반-방법-vs-전통적-방법) | - 언어적 복잡성 및 애매모호함<br>- 스키마 이해 및 표현<br>- 복잡하거나 드문 SQL 연산<br>- Cross-Domain 에 대한 일반화 |
| [LLM 기반 방법 간 비교](#4-2-llm-기반-방법-간-비교)            | - ICL 기반 방법 간 비교<br>- Fine-Tuning 기반 방법 간 비교<br>- ICL 방법과 Fine-Tuning 방법 간 비교      |
| [실제 적용 시의 Robustness](#4-3-실제-적용-시의-robustness)  | - 실제 응용 프로그램 적용 시의 Robustness                                                      |
| [데이터 보안 및 해석 가능성](#4-4-데이터-보안-및-해석-가능성)          | - 데이터 보안<br>- 데이터 해석 가능성                                                           |

### 4-1. LLM 기반 방법 vs. 전통적 방법

이 부분의 논의 사항은 다음과 같다.

* LLM 기반 방법들은 Rule-based 방법에 비해 **여러 가지 측면에서 장점이 많음** 을 알 수 있다.

| 논의 사항                 | 설명                                                                                                                               |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------|
| 언어적 복잡성 및 애매모호함       | 전통적인 Rule-based 시스템들과 달리, LLM 기반 시스템 (DAIL-SQL 등) 은 **Chain-of-thought Prompting + 보다 넓은 범위의 Pre-training** 등을 이용하여 이를 해결할 수 있다. |
| 스키마 이해 및 표현           | LLM 기반 방법은 **in-context learning 및 structured attention** 을 통해 schema linking 능력이 향상되었다.                                         |
| 복잡하거나 드문 SQL 연산       | LLM 기반 방법들은 code-centric pre-training 을 이용하여 **parametric knowledge 를 결합하여 자주 등장하지 않는 SQL 문을 생성** 한다.                            |
| Cross-Domain 에 대한 일반화 | Rule-based 방법들은 정해진 템플릿에 맞춰야 하지만, LLM 기반 방법들은 code-centric pre-training 을 통해 이를 해결한다.                                            |

### 4-2. LLM 기반 방법 간 비교

* LLM 기반 방법으로는 **ICL (in-context learning) 패러다임, FT (Fine-Tuning) 패러다임, 그리고 이를 결합한 것** 이 있다.

| LLM 기반 패러다임                    | 설명                                                                                                                                  |
|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| ICL (in-context learning) 패러다임 | - **Decomposition**: 복잡한 쿼리 관리에 효과적이지만 Multi-stage pipeline 에서 오류 가능성<br>- **Prompt Optimization**: 입력 품질을 향상시키지만 내부적 추론 능력의 한계가 있음 |
| FT (Fine-Tuning) 패러다임          | 특화 (specialization) 와 일반화 (generalization) 간의 trade-off                                                                             |
| ICL & FT 결합 패러다임               | - ICL 기반 방법들은 **오픈소스가 아닌 LLM (GPT-4 등)** 에 적합<br>- Fine-Tuning 기반 방법들은 **오픈소스 LLM** 에 적합                                            |

### 4-3. 실제 적용 시의 Robustness

* 사용자는 **항상 분명한 질문을 하지는 않으므로**, 해당 부분은 명백한 도전 과제이다.
* Robustness-specific 데이터셋 분야는 발전했지만, **실제 활용 시에는 성능이 떨어질 수 있다.**
* 추가 문제점
  * local text-to-SQL 데이터셋으로 Fine-Tuning 된 LLM은 **비표준화된 sample 과 label** 이 있을 수 있다.
  * 실제 현장에서의 데이터셋 크기는 **연구 벤치마크의 데이터셋 크기보다 작다.**
  * **다국어 & 멀티모달 시나리오** 도 고려 사항이다.

### 4-4. 데이터 보안 및 해석 가능성

* 가장 중요한 도전 과제는 다음과 같다.
  * **로컬 DB의 데이터가 유출되지 않도록** 해야 한다. (데이터 프라이버시)
  * 이를 위해 proprietary API를 호출하여 로컬 DB를 관리해야 한다.
* 해석 가능성 관련 문제는 다음과 같다.
  * text-to-SQL 분야에서, **LLM 기반 구현의 '해석 가능성'은 아직 잘 논의되지 않고 있다.** 