...
* 본 논문에서 제안하는 **Reinforce-Rej** 는 다음과 같은 컨셉이다.
  * Reinforce **+ Remove both (correct & incorrect samples)** 
...
**2. Reward, KL Loss, Entropy Loss 비교**

* 비교 대상
  * Reinforce vs. Reinforce 에 각종 요소 추가 시
  * GRPO vs. Reinforce vs. **Reinforce-rej (ours)** vs. RAFT++
...
* 여기서는 LLM post-training 시의 강화학습 방법에 대해 **Rejection Sampling** 관점에서 접근했다.
* 여기서 소개한 RAFT 는 다음과 같다.
  * **positive reward sample** 에만 의존하는 rejection 기반 방법론
  * PPO, iterative DPO 등 **기존에 알려진 더 정교한 방법론들보다 우수한 성능**
* RAFT++ 는 이 RAFT 에 **importance sampling & clipping** 을 추가로 적용한 것이다.
