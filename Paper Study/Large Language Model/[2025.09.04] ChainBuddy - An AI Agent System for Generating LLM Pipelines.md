...
| 질적 실험 | - ChainBuddy 는 사용자의 'blank page problem'을 극복하게 했음<br>- ChainBuddy 의 요구사항 분석 프로세스는 **참가자가 구체적인 목표 및 기준을 제시** 하도록 지원했음 |
...
| 실험 항목                                 | 결론                                                   |
|---------------------------------------|------------------------------------------------------|
| Time to completion (H1)               | 양쪽 간 큰 차이 없음                                         |  
| Workload Demand (NASA TLX) (H2)       | ChainBuddy 를 사용한 쪽이 **정신적 및 물리적 부담이 덜했음**            |      
| Perceived System Usability (H3)       | ChainBuddy Assistant 를 사용할 때 **Confidence** 가 훨씬 높았음 |           
| Performance (H4)                      | ChainBuddy 를 사용한 쪽이 **유의미한 성능 향상** 이 있었음             |      
| Number of Types of Created Nodes (H5) | ChainBuddy 를 사용한 쪽이 **생성한 node 의 type 가 훨씬 더 다양** 했음 |

* **2. 실험 결과 종합 그래프**

![image](../images/LLM_ChainBuddy_4.PNG)

[(출처)](https://arxiv.org/pdf/2409.13588) : Jingyue Zhang and Ian Arawjo et al., "ChainBuddy: An AI Agent System for Generating LLM Pipelines"

* **3. (expert ratings 기준) User performance**

![image](../images/LLM_ChainBuddy_5.PNG)
...
| 실험 항목                          | 결론                                                                                    |
|--------------------------------|---------------------------------------------------------------------------------------|
| user 의 'blank page problem' 극복 | ChainBuddy 는 사용자가 해당 문제를 **극복하는 것을 돕는 데 성공** 함                                        |
| 사용자 요구사항 추출                    | ChainBuddy 는 **참가자가 사용자 요구사항 (목표 & 기준)** 을 적절히 추출하도록 돕는 데 성공함                         |
| 참가자의 업무량 감소                    | ChainBuddy 는 참가자가 **업무량을 감소시켜 개발을 빠르게** 진행할 수 있게 함                                    |
| vs. 참가자의 기대                    | ChainBuddy 는 **참가자의 기대를 뛰어넘는 결과** 를 보였음<br>- 이는 **사용자를 놀라게 했다** 는 점에서 사용자에 대해서도 마찬가지임 |
| 참가자의 프롬프트 엔지니어링 경험의 영향         | 프롬프트 엔지니어링 경험이 있었던 참가자들은 **ChainBuddy가 프롬프트 엔지니어링 프로세스에 도움을 주었다** 고 판단                |
| 잠재적 문제점                        | ChainBuddy 의 잠재적 문제점 (편견 등) 에 대한 우려가 발생하기도 했음                                         |

blank page problem : 대략적인 아이디어를 구체적인 workflow 로 변환하는 것의 어려움

* **2. 참가자의 Interaction Trace (with vs. w/o ChainBuddy)**

![image](../images/LLM_ChainBuddy_6.PNG)
...
| 논의 사항                                       | 설명                                                                                                                                                                                         |
|---------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| LLM Pipeline 생성에 있어서 AI Assistant 에게 과도한 의존 | - 본 연구에서 ChainBuddy가 잘못된 Solution을 도출한 Error Case를 1가지 봤다.<br>- 해당 case의 참가자는 **AI의 실수를 찾거나 고칠 수 없었다.**<br>- 즉, **미래의 AI Assistant가 bias를 줄이면서도 어떻게 하면 사용자가 인터페이스를 알 수 있게 할지?** 가 질문 사항이다. |
| 사용자 요구사항 수집을 위한 올바른 상호작용 설계                 | - 서로 다른 인터페이스에서 사용자의 의도 사항을 추출하는 부분에 있어서, **각 인터페이스 별로 무엇이 trade-off** 인지 정의하기 어렵다.<br>- form-filling 구조는 채팅 방식의 AI Assistant 환경에서는 어색할 수 있다고 느껴진다.                                        |
| 향후 과제                                       | - AI Assistant 와의 채팅에서 **외부 데이터 연계**<br>- 기존 Chain의 수정 기능 지원<br>- AI의 의사결정에 대한 **사용자 조정 권한** 제공<br>- 다양한 Chain에 대한 지원                                                                      |
