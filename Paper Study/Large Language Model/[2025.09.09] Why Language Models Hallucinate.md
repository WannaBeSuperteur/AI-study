...
본 논문에서의 논의 사항은 다음과 같다.

* 환각 현상을 정의하고, 평가하고, 제거하는 것은 **환각 현상이 본질적으로 다측면적인 특성** 을 가지고 있기 때문에 어렵다.
* LLM의 환각 현상 정의, 평가, 제거를 위한 프레임워크의 **한계점 및 확장성** 은 다음과 같다.

| 한계점 / 확장성                 | 설명                                                                                                                                                                                                                                                  |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 그럴듯하지만 말이 안 되는 것          | 환각 현상은 **그럴듯하지만 실제로는 잘못된 정보** 인 경우가 많다.<br>- 본 연구에서는 그럴듯한 문자열의 집합 $X$ 만을 고려하므로, **잘못된 정보가 포함된 string 이 생성될 가능성을 고려하지 않았다.**                                                                                                                         |
| Open-ended generation     | 환각 현상은 **open-ended generation 을 요구하는 프롬프트** 에서도 발생한다.<br>- ```Write a biography about ...```                                                                                                                                                       |
| RAG 과 같은 검색은 완벽하지 않음      | LLM은 [RAG (Retrieval-Augmented Generation)](../../AI%20Basics/LLM%20Basics/LLM_기초_RAG.md) 과 같은 **검색 기법** 과 결합하면 환각 현상을 줄일 수 있다고 알려져 있다.<br>- 그러나 **위에서 말한 binary grading system** 의 문제점은 **검색이 실패했지만 confident answer 를 생성하는 경우에도 여전히 reward가 높다.** |
| 내재적 문맥 (Latent Context)   | 어떤 오류는 **맥락을 파악해야 하기** 때문에, **prompt & response 의 쌍을 이용하여 판단** 하기 어렵다.                                                                                                                                                                              |
| LLM 답변을 3가지로 구분해도 완벽하지 않음 | - 본 연구에서는 LLM의 답변의 **오류의 심각성 수준을 구체적으로 나누지는** 않는다.<br>- LLM의 답변을 ```correct``` ```incorrect``` ```I don't know``` 로 구분하더라도 완벽하지 않다.                                                                                                                 |
| LLM 답변의 불확실성을 구분하지 않음     | LLM의 답변에는 **수많은 불확실성의 신호 (상세 사항 누락, 추가 질문 등)** 가 있지만, 본 연구에서는 그것들에 대해 구체적으로 연구하지는 않음                                                                                                                                                                | 
