...
![image](../images/EquivalentEncryption_8.png)
...
![image](../images/EquivalentEncryption_9.png)
...
| 분류                     | 설명                                                                                                                                                          |
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Partial HE (PHE)       | 덧셈, 곱셈과 같은 단일 연산 중 **한번에 하나만 실행 가능**                                                                                                                        |
...
* 공격자는 등가 암호화에 의해 암호화된 입출력 $I_i, O_i \in C$ 에 대한 입출력 순서 $\lbrace(I_i, O_i)\rbrace_{i=1}^n$ 를 관측한다.
* 공격자의 상황
...
다음과 같이 **Fidelity Score** 를 통해, **2회의 inference 에 의해 생성된 logit 의 유사성** 에 대한 **confidence value** 를 측정한다.

* Fidelity score가 클수록 **EE (등가 암호화)** 를 적용한 모델의 confidence score가 **Vanilla Inference 적용 모델과 유사** 함을 의미한다.

![image](../images/EquivalentEncryption_6.png)
...
|------------|--------------------------------------------------------------------------------------|
| $n$        | 전체 샘플 개수                                                                             |
| $s_i^{VI}$ | $i$번째 sample 에 대한 **class/first token confidence** 점수 (**Vanilla Inference** 기반)     |
| $s_i^{EE}$ | $i$번째 sample 에 대한 **class/first token confidence** 점수 (**Equivalent Encryption** 기반) |

![image](../images/EquivalentEncryption_7.png)
