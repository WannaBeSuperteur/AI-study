...
| 입력값        | 입력값 설명                                           | 입력값 결정 모델              | 입력값 결정 모델 Class                 | 입력값 결정 모델을 위한, 학습 데이터 output      | 입력값 결정 모델이 판정한, 모든 이미지에 대한 output | 
|------------|--------------------------------------------------|------------------------|---------------------------------|-----------------------------------|-----------------------------------|
| eyes       | **눈을 뜬 정도 👀**<br>0~1                            | ```input_eyes```       | ```model_input_eyes.py```       | ```train_output_eyes.csv```       | ```all_output_eyes.csv```                    |
| hair_color | **헤어 컬러 👩‍🦰**<br>0~1, 진할수록 1에 가까움              | ```input_hair_color``` | ```model_input_hair_color.py``` | ```train_output_hair_color.csv``` | ```all_output_hair_color.csv``` |
| mouth      | **입을 벌린 정도 👄**<br>0~1                           | ```input_mouth```      | ```model_input_mouth.py```      | ```train_output_mouth.csv```      | ```all_output_mouth.csv```         |
| background | **배경의 밝기 🏞**<br>0~1, 밝을수록 1에 가까움                | ```input_background``` | ```model_input_background.py``` | ```train_output_background.csv``` | ```all_output_background.csv```   |
| head       | **고개 돌림 유형 🐴**<br>0 (왼쪽), 1 (오른쪽), 0.5 (돌리지 않음) | ```input_head```       | ```model_input_head.py```       |  ```train_output_head.csv```      | ```all_output_head.csv```          |
