...
class CVAE_Model:
    ...
    def __init__(self, dropout_rate=0.45):
        ...
        self.encoder_cnn1 = layers.Conv2D(64, (4, 4), strides=2, activation=silu, padding='same', kernel_regularizer=L2, name='ec1')
        self.encoder_bn_cnn1 = layers.BatchNormalization(name='ec1_bn')
        self.encoder_ac_cnn1 = layers.Activation(silu, name='ec1_ac')

        self.encoder_cnn2 = layers.Conv2D(128, (4, 4), strides=2, activation=silu, padding='same', kernel_regularizer=L2, name='ec2')
        self.encoder_bn_cnn2 = layers.BatchNormalization(name='ec2_bn')
        self.encoder_ac_cnn2 = layers.Activation(silu, name='ec2_ac')

        self.encoder_cnn3 = layers.Conv2D(256, (4, 4), strides=1, activation=silu, padding='same', kernel_regularizer=L2, name='ec3')
        self.encoder_bn_cnn3 = layers.BatchNormalization(name='ec3_bn')
        self.encoder_ac_cnn3 = layers.Activation(silu, name='ec3_ac')

        self.encoder_dense0 = layers.Dense(256, activation=silu, name='ed0')
        ...
        self.decoder_dense1 = layers.Dense(256 * TOTAL_CELLS // (8 * 8), activation=silu, name='dd1')

        self.decoder_cnn0 = layers.Conv2DTranspose(128, (4, 4), strides=2, activation=silu, padding='same', kernel_regularizer=L2, name='dc0')
        self.decoder_bn_cnn0 = layers.BatchNormalization(name='dc0_bn')
        self.decoder_ac_cnn0 = layers.Activation(silu, name='dc0_ac')

        self.decoder_cnn1 = layers.Conv2DTranspose(64, (4, 4), strides=2, activation=silu, padding='same', kernel_regularizer=L2, name='dc1')
        self.decoder_bn_cnn1 = layers.BatchNormalization(name='dc1_bn')
        self.decoder_ac_cnn1 = layers.Activation(silu, name='dc1_ac')

        self.decoder_cnn2 = layers.Conv2DTranspose(32, (4, 4), strides=2, activation=silu, padding='same', kernel_regularizer=L2, name='dc2')
...
#        input_image_reshaped = layers.Reshape((INPUT_IMG_SIZE, INPUT_IMG_SIZE, NUM_CHANNELS))(input_image)
        ...
        dec_reshaped = layers.Reshape((INPUT_IMG_SIZE // 8, INPUT_IMG_SIZE // 8, 256))(dec_d1)

        # decoder additional
        dec_add_c0 = layers.Dense((INPUT_IMG_SIZE // 8) * (INPUT_IMG_SIZE // 8) * 256, name='dec_c0_add', activation=silu)(dec_merged)
        dec_add_c0_ = layers.Reshape((INPUT_IMG_SIZE // 8, INPUT_IMG_SIZE // 8, 256))(dec_add_c0)

        dec_add_c1 = layers.Dense((INPUT_IMG_SIZE // 4) * (INPUT_IMG_SIZE // 4) * 128, name='dec_c1_add', activation=silu)(dec_merged)
        dec_add_c1_ = layers.Reshape((INPUT_IMG_SIZE // 4, INPUT_IMG_SIZE // 4, 128))(dec_add_c1)

        dec_add_c2 = layers.Dense((INPUT_IMG_SIZE // 2) * (INPUT_IMG_SIZE // 2) * 64, name='dec_c2_add', activation=silu)(dec_merged)
        dec_add_c2_ = layers.Reshape((INPUT_IMG_SIZE // 2, INPUT_IMG_SIZE // 2, 64))(dec_add_c2)

        dec_add_c3 = layers.Dense(INPUT_IMG_SIZE * INPUT_IMG_SIZE * 32, name='dec_c3_add', activation=silu)(dec_merged)
        dec_add_c3_ = layers.Reshape((INPUT_IMG_SIZE, INPUT_IMG_SIZE, 32))(dec_add_c3)
