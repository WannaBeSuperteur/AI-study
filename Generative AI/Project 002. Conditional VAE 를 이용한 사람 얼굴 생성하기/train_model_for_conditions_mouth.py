...
class Regression_Mouth_Model(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        # conv + pooling part : 60 -> 58 -> 56 -> 28 -> 26 -> 13 -> 11 -> 9
        outputs_0 = self.conv_0(inputs_img) # 58
        outputs_1 = self.conv_1(outputs_0) # 56
        outputs_2 = self.pooling(outputs_1) # 28

        outputs_3 = self.conv_2(outputs_2) # 26
        outputs_4 = self.pooling(outputs_3) # 13

        outputs_5 = self.conv_3(outputs_4) # 11
        outputs_6 = self.conv_4(outputs_5) # 9
...
# 가로로 위쪽에서 2, 3번째 줄과 세로로 위쪽에서 3, 4번째 줄의 교차점 (16등분 중 총 4개 영역) 에 해당하는 부분만 있는 이미지로 crop
def crop_images(all_images):
    img_count = len(all_images)
    cropped_all_images = np.zeros((img_count, IMG_SIZE, IMG_SIZE, 3))
    
    x_start = (ORIGINAL_IMG_SIZE - IMG_SIZE) // 2
    y_start = ORIGINAL_IMG_SIZE - IMG_SIZE
    
    for i in range(img_count):
        cropped_all_images[i] = all_images[i][y_start : y_start+IMG_SIZE, x_start : x_start+IMG_SIZE]
...
if __name__ == '__main__':
    ...
    if 'regression_mouth' in os.listdir():
        cnn_model = tf.keras.models.load_model('regression_mouth')
        ...
        # CNN 모델 저장
        cnn_model.save('regression_mouth')
