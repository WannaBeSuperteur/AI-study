...
class Regression_Background_Mean_Model(tf.keras.Model):
...
def define_model():
    ...
    model = Regression_Background_Mean_Model()
...
def load_training_data():
    ...
    background_mean_male = read_output('regression_background_mean_info_male.csv')
    background_mean_female = read_output('regression_background_mean_info_female.csv')
    all_background_mean_values = np.concatenate((background_mean_male, background_mean_female), axis=0)

    train_input_img = np.array(all_images)
    train_input_gender_prob = get_gender_prob_data(male_img_names, female_img_names)
    train_output = np.array(all_background_mean_values)
...
if __name__ == '__main__':
    ...
    if 'regression_background_mean' in os.listdir():
        cnn_model = tf.keras.models.load_model('regression_background_mean')
        ...
        # CNN 모델 저장
        cnn_model.save('regression_background_mean')
